...
class AssociationMatrix(nn.Module):
    ...
    def forward(self, xs, xt):
        ...
        Pst = F.softmax(W, dim=1)
        Pts = F.softmax(W.transpose(1,0), dim=1)

        Psts = Pst.mm(Pts)

        Pt = torch.mean(Pst, dim=0, keepdim=True)
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        phi  = self.features(x)
        phi_mean = phi.view(-1, 128, 9).mean(dim=-1)
        phi = phi.view(-1,128*3*3)
